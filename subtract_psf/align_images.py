...
class AlignImages:
    ...
    Reads in data cubes from a directory of FITS images of "observations"
    generated by CreateImages() and does its best to make sure the background
    star is located at the same spot in all images, whether they're from a
    reference or target data cube. The rough roadmap is to pad the data cubes
    with NaNs, shift them, remove the padding, and pass them on to
    SubtractImages() as the self.stackable_cubes HDUList.

    The padding and shifting is done by self._pad_and_shift_cubes(), but the
    determination of the shifts themselves depends on self.align_style, which
    is set as one of KlipRetrieve's keyword arguments.

    If self.align_style == 'empirical2' (the default):
        - self._align_brights_empirical() locates the brightest pixel in an
        image from each data cube and shifts all images so that their bright
        pixels occur in the same pre-determined location.

        - self._finalize_emp2_cubes() finds the pixel coordinates that are non-
        NaN in every image and slices them down so that no padding remains.

    If self.align_style == 'theoretical':
        - self._align_brights_theoretical() removes each observation's pointing
        error and pre-designated dither offset to bring the target star as
        close to the middle of the center pixel as possible.

        - self._finalize_theo_cubes() makes a copy of all the shifted reference
        cubes to go with each target cube. Each target's reference cubes are
        "fine shifted" by a pixel in either axis (if necessary) to match that
        specific target's location as closely as possible. (This means that
        unlike with 'empirical2', not all target images will have the same post-
        alignment shape.) The method uses self._fine_shift_theo_cubes() to
        perform the fine shifts and remove all padding.
    '''
    ...
    def _pad_and_shift_cubes(self, cube_imgs, shifts, buffer=True):
        ...
        to the images in each cube. It should be the same length as
        `cube_imgs`; all slices of a given data cube are shifted by the same
        amount. The amount of padding in the returned array depends on the
        maximum value in both axes of `shifts`.
    ...
    def _align_brights_theoretical(self, cube_list, verbose=True):
        ...
        steps and overall pointing errors. (Takes from the former
    ...
    def _finalize_emp2_cubes(self, cube_list, verbose=True):
        '''
        Slices out any of a data cube's slices' outer rows/columns that contain
        NaNs. Adapted from former `self._get_best_pixels()` method.

        Argument `cube_list` is an HDUList of padded data cubes that went
        through the 'empirical2' alignment process.
    ...
    def _finalize_theo_cubes(self, cube_list, verbose=True):
        '''
        Completes the theoretical alignment process and removes padding from
        all images. It returns the edited HDUList along with two lists that are
        necessary due to intricacies of the theoretical alignment process.
